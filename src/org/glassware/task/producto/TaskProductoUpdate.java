 ...
 * @author LiveGrios
 */
public class TaskProductoUpdate extends Task<Void>
{
    ...
    public TaskProductoUpdate(WindowMain app, PanelProducto panelProducto, Producto producto)
    {
        this.app = app;
    ...
    {
        String server = MySPACommons.URL_SERVER + "/api/Producto/update?";
         ...
         try{
            if(jso.has("result")){
                if (jso.get("result").getAsString().toLowerCase().equals("exito")) {
                    app.showAlert("Movimiento realizado", "Se modifico el registro correctamente", Alert.AlertType.INFORMATION);
